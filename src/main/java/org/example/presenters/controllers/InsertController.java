...
@RestController
@RequestMapping("/messenger/insert")
public class InsertController {
    ...
    @Autowired
    public InsertController(UserAccountService userAccountService, MessageService messageService,
    ...
    public ResponseEntity<Object> insertUserAccount(@RequestBody UserAccount userAccount) {
        UserCommand command = new AddUserAccountCommand(userAccount, this.userAccountService);
    ...
    public ResponseEntity<Object> insertMessage(@RequestBody Message message) {
        MessageCommand command = new AddMessageCommand(message, messageService);
        ResponseCodes response = (ResponseCodes) messageInvoker.doCommand(command);
        return new ResponseEntity<>(response.toString(), HttpStatusCode.valueOf(200));
    }

    @RequestMapping(method = RequestMethod.POST, value = "/chat")
    public ResponseEntity<Object> insertChat(@RequestBody Chat chat) {
        ChatCommand command = new AddChatCommand(chat, chatService);
        ResponseCodes response = (ResponseCodes) chatInvoker.doCommand(command);
        return new ResponseEntity<>(response.toString(), HttpStatusCode.valueOf(200));
    }

    @RequestMapping(method = RequestMethod.POST, value = "/members")
    public ResponseEntity<Object> insertMember(@RequestBody GroupMembers members) {
        GroupMembersCommand command = new AddGroupMembersCommand(members, groupMembersService);
        ResponseCodes response = (ResponseCodes) groupMembersInvoker.doCommand(command);
        return new ResponseEntity<>(response.toString(), HttpStatusCode.valueOf(200));
    }

    @RequestMapping(method = RequestMethod.POST, value = "/admin")
    public ResponseEntity<Object> insertAdminGroup(@RequestBody AdminGroup adminGroup) {
        AdminGroupCommand command = new AddAdminGroupCommand(adminGroup, adminGroupService);
    ...
    public ResponseEntity<Object> insertContact(@RequestBody ContactList contactList) {
        ContactListCommand command = new AddContactListCommand(contactList, contactListService);
